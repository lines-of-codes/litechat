...
async function processChatData(
	...
	const chatNames: StringMap = {};
	const chatKeys: { [id: string]: CryptoKey } = {};
	const undecryptableChat = [];
...
async function exportJson(
	...
	const output: {
		...
		chats?: ProcessedChat[];
		encryptionKeys?: Record<string, string | Uint8Array<ArrayBufferLike>>;
		relatedPeople?: ProcessedRelated[];
	} = {
		options: {
	...
	const users: { [id: string]: UserModel } = userList.users;
	const userNames: StringMap = userList.userNames;

	const chatData = await processChatData(userData, users);

	output.undecryptableChat = chatData.undecryptableChat;
	const chatNames = chatData.chatNames;
	const chatKeys = chatData.chatKeys;
	...
	if (options.hasRelatedPeople) {
		...
		output.relatedPeople = await Promise.all(
			userData.relatedPeople.map(async (user) => {
				const processed: ProcessedRelated = {
					avatar: user.avatar,
					id: user.id,
					name: user.name,
	...
	switch (options.keyFormat) {
		...
		case "notJwk": {
			const files: { [name: string]: ZippableFile } =
				output.encryptionKeys as Record<
					string,
...
async function exportMarkdown(
	...
	const encryptionKeys: { [id: string]: string } = {};
...
async function exportPlainText(userData: UserData): Promise<ExportResult> {
	...
	for (const chat of userData.chats) {
		...
		for (const message of messages) {
			...
			const sender = userList.userNames[message.sender];
			let editedOn = "";
...
async function exportData(formData: FormData) {
	...
	try {
		...
		const userData = await pb.send(
			`/api/litechat/users/export/${thisUserId}`,
			{},
		);

		const parsedOptions: ExportOptions = {
		...
		const urlBlob = URL.createObjectURL(result.blob);
		const date = new Date();
